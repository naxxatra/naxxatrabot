...
def apod_get():
    ...
    embed.set_author(name="NASA APOD", url="https://apod.nasa.gov/",
                     icon_url="https://upload.wikimedia.org/wikipedia/commons/thumb/e/e5/NASA_logo.svg/1237px-NASA_logo.svg.png")
...
async def search(ctx, *, arg):
    ...
    embed.set_author(name="Wolfram Alpha", url="https://www.wolframalpha.com/",
                     icon_url="https://i.imgur.com/uX2g1SL.png")
...
@bot.command()
async def make_poll(ctx, question, opt_a, opt_b, opt_c="", opt_d="", opt_e="",
                    opt_f=""):  # This can make polls upto 6 options long. Needs minimum of two options
    ...
    rgb = (random.randint(0, 255), random.randint(0, 255),
           random.randint(0, 255))  # Randomly generates colour each time the poll is generated.
    embed = discord.Embed(title=question,
                          description=body,
                          colour=discord.Color.from_rgb(rgb[0], rgb[1], rgb[2]))  # making the embed
    msg = await ctx.send(embed=embed)  # Sending the message

    option_emojis = ['\U0001f1e6', "\U0001f1e7", "\U0001f1e8", "\U0001f1e9", "\U0001f1ea",
                     "\U0001f1eb"]  # Emojis from :regional_indicator_a: to :regional_indicator_f:
    for i in range(len(options)):
...
async def xkcd_today(ctx):
    ...
    colour = discord.Colour.from_rgb(random.randint(0, 255), random.randint(0, 255),
                                     random.randint(0, 255))  # Randomly generating a colour

    embed = discord.Embed(title=today["safe_title"],
                          color=colour)  # Making an embed, with the title being today's xkcd comic title.
    embed.set_author(name="XKCD", url=r"https://xkcd.com/{}".format(comic_num))  # Setting the author to be xkcd, and linking to the website
...
async def xkcd_random(ctx):
    ...
    response = requests.get(
        "https://xkcd.com/{num}/info.0.json".format(num=comic_num))  # Getting comic with the random number
    ...
    colour = discord.Colour.from_rgb(random.randint(0, 255), random.randint(0, 255),
                                     random.randint(0, 255))  # Randomly generating a colour
    embed = discord.Embed(title=comic["safe_title"], description="XKCD Number: {num}".format(num=comic_num),
                          color=colour)  # Making an embed
    embed.set_author(name="XKCD", url="https://xkcd.com/{}".format(comic_num))  # Setting the author
...
async def help(ctx, *, params=None):
    if params == "youtube" or params == "yt":
        embed = discord.Embed(title="Naxxatra Bot Help", colour=discord.Colour(0xff0000),
                              description="***Youtube Together***")

        embed.set_image(url="https://i.imgur.com/J5EETHu.png")
        embed.set_thumbnail(
            url="https://cdn.discordapp.com/icons/761896483838492682/756a7be666993d92c2421dd8af6d21ef.webp")

        embed.add_field(name="How to create a new watch party/youtube together:",
                        value="\\> Join a VC (voice chat)\n\\> Run the command( `n!youtube` or  `n!yt` or you can also use the slash command `/youtube`)\n\\> Click on the blue hyperlink that the bot sent\nYou have successfully created the watch party/Youtube together",
                        inline=False)
        embed.add_field(name="How To Use YouTube Together in a VC:",
                        value="\\>  For the first time it will ask for authorization. You should authorize it. \n\\> After that it will take a few minutes to load.\n\\> You can then search for a YouTube video or paste the link.",
                        inline=False)
        embed.add_field(name="How To Join an existing Watch Party/ Youtube Together:",
                        value="\\> If someone has started the watch party in a VC then you can just hover your mouse over that channel & you will see a button to join activity.\n**OR**\n\\> You could just click on the previous hyperlink that the bot sent initially.",
                        inline=False)
        await ctx.send(embed=embed)
    else:
        ...
        embed.add_field(name="`n!apod`", value="This command gives you the NASA's APOD (Astronomy Picture Of The Day)",
                        inline=False)
        embed.add_field(name="`n!trivia <number>`",
                        value="This command gives you a trivia fact about the number entered or a random number if no number is given",
                        inline=False)
        embed.add_field(name="`n!search [question]`",
                        value="Get answers to almost everything. Powered by Wolfram Alpha API", inline=False)
        ...
        embed.add_field(name="***SLASH COMMANDS***",
                        value="All the above commands can also be used through Slash Commands. \nFor eg. You can use `/apod` for using n!apod command. \nSlash commands are easier to use.\n\n\n[Website](https://naxxatra.com/)\n[Github](https://github.com/naxxatra/naxxatrabot)",
                        inline=False)
